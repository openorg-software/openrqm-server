...
import io.swagger.api.WorkspaceApi;
...
public class WorkspaceApiController implements WorkspaceApi {
    ...
    public ResponseEntity<RQMWorkspace> getWorkspace() {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<RQMWorkspace>(objectMapper
                        .readValue("{  \"workspace_id\" : 6,  \"name\" : \"name\",  \"id\" : 0}", RQMWorkspace.class),
                        HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<RQMWorkspace>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<RQMWorkspace>(HttpStatus.NOT_IMPLEMENTED);
    }
